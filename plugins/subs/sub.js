const run = async (m, { args, conn, bot }) => {
  ...
  if (global.subBots && global.subBots.list().length >= 25) {
    return m.reply(`نأسف لإيقاف الاستقبال.. الأداء فوق الجميع! 🚫🧠
تم اكتمال البوتات الفرعية بنجاح. لازم تنتظر خروج حد من المنصبين الحاليين عشان تعرف تربط البوت، غير كده مش هتعرف تنصب. بنحمي السيرفر عشان ميهنجش بسبب الزحمة. عذرًا.. 👑`);
  }

  if (global.db.noSub) return m.reply(`نأسف لإيقاف الاستقبال.. الأداء فوق الجميع! 🚫🧠
تم اكتمال البوتات الفرعية بنجاح. لازم تنتظر خروج حد من المنصبين الحاليين عشان تعرف تربط البوت، غير كده مش هتعرف تنصب. بنحمي السيرفر عشان ميهنجش بسبب الزحمة. عذرًا.. 👑`);
  ...
  try {
    ...
    const init = await m.reply(`⏳ جاري رابط بوت للرقم *${num}*...`);
...
const Func = {
  pair: async (conn, code, num, m, reply_status) => {
    await conn.sendButton(m.chat, {
      imageUrl: "https://files.catbox.moe/2xhp9q.jpg",
...
> *_افتح واتساب > الأجهزة المرتبطة > ربط جهاز برقم الهاتف > أدخل الكود_*`,
      ...
      buttons: [
        ...
        { name: "cta_url", params: { display_text: "⟨🃏| 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 𝑲𝑰𝑵𝑮 𝑩𝑶𝑻 |🃏⟩", url: "https://whatsapp.com/channel/0029VbCPEZ88F2pNsd7YQB1i" } },
      ],
      mentions: [m.sender],
      newsletter: {
        name: '𝑲𝑰𝑵𝑮 𝑩𝑶𝑻',
